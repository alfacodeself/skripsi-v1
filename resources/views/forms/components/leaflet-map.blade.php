...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field" lazy>
    <!-- Container utama peta dengan Alpine.js data binding -->
    <div x-data="{
        latitude: @entangle($getStatePath() . '.latitude').live,
        longitude: @entangle($getStatePath() . '.longitude').live,
        ...
        initMap() {
            ...
            this.$watch('latitude', (value) => this.marker.setLatLng([value, this.longitude]));
            this.$watch('longitude', (value) => this.marker.setLatLng([this.latitude, value]));
        }
    }" x-init="initMap" wire:ignore wire:key="{{ $field->getId() }}"
        style="position: relative; z-index: 0;" class="w-full rounded-lg shadow">
    </div>
</x-dynamic-component>
